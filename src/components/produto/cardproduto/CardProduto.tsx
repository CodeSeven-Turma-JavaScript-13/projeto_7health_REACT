    ...
    <div className="group relative bg-white border border-gray-100 rounded-4xl overflow-hidden hover:shadow-2xl transition-all duration-500 flex flex-col h-full shadow-sm">
      <div className="relative h-56 overflow-hidden">
        ...
        <div className="absolute top-4 right-4 flex gap-2 translate-y-2 opacity-0 group-hover:translate-y-0 group-hover:opacity-100 transition-all duration-300">
          <Link to={`/editarProduto/${produto.id}`} className="p-2 bg-white/90 backdrop-blur-sm rounded-xl text-brand-dark hover:bg-brand-medium hover:text-white transition-colors">
            <Edit size={18} />
          </Link>
          <Link to={`/deletarProduto/${produto.id}`} className="p-2 bg-white/90 backdrop-blur-sm rounded-xl text-red-500 hover:bg-red-50 transition-colors">
            <Trash2 size={18} />
          </Link>
        </div>
      ...
      <div className="p-6 flex flex-col grow">
        <div className="flex justify-between items-start mb-2">
          <h3 className="text-xl font-bold text-[#1a3c1a]">{produto.nome}</h3>
          <span className="text-xl font-bold text-brand-medium">
            {new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(produto.preco)}
          </span>
        </div>
        <p className="text-[#4a5c4a] text-sm line-clamp-2 mb-4 grow">{produto.descricao}</p>
        <div className="flex items-center justify-between pt-4 border-t border-gray-100 mt-auto">
          ...
          <button 
            ...
            className="flex justify-center items-center px-4 py-2 bg-brand-dark text-white text-xs font-bold rounded-lg hover:bg-brand-medium transition-all duration-300"
          >
